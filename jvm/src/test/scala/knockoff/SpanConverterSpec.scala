...
class SpanConverterSpec extends AnyFunSpec with Matchers {
  ...
  describe("SpanConverter") {
    ...
    it("should match emphasis underscores containing asterix emphases") {
      ...
      convert(txt) should equal {
        List(Text("an "), Emphasis(List(Text("underscore "), Emphasis(Text("and block") :: Nil), Text(" em"))))
      }
    }
    ...
    it("should match strong underscores containing asterix emphases") {
      ...
      convert(txt) should equal {
        List(Text("an "), Strong(List(Text("underscore "), Strong(List(Text("and asterix"))), Text(" strong"))))
      }
    }
    ...
    it("should find a couple of entities and pass them through") {
      ...
      convert(txt) should equal {
        List(Text("an "), HTMLSpan("&amp;"), Text(" and an "), HTMLSpan("&em;"), Text(" are in here"))
      }
    }
    ...
    it("should discover inline, image, automatic, and reference links") {
      val defs = List(new LinkDefinitionChunk("link1", "http://example.com", Some("title")))
      ...
      convert(txt, defs) should equal {
        List(
          ...
          Text(" An "),
          ImageLink(List(Text("image link")), "http://example.com/image1", Some("image test")),
          Text(" The "),
          Link(List(Text("http://example.com/automatic")), "http://example.com/automatic", None),
          Text(" A "),
          Link(List(Text("reference link")), "http://example.com", Some("title"))
        )
      }
    }
    ...
    it("should hande link references in different case") {
      ...
      convert(txt, defs) should equal {
        List(
          Link(List(Text("Link 1")), "http://example.com/1", None),
          Text(" and "),
    ...
    it("should ignore backslashes in code") {
      ...
      convert(txt) should equal(List(Text("""a backslash """), CodeSpan("\\"), Text(""" in code""")))
    }

    it("should parse links ending in brackets") {
      val text = """a [link](http://example.com/path_(foo))"""
      val parsed = List(Text("a "), Link(List(Text("link")), "http://example.com/path_(foo)", None))
