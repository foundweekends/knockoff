...
trait XHTMLWriter {
  ...
  def spanToXHTML: Span => Node = span =>
    span match {
  ...
  def linkToXHTML: (collection.Seq[Span], String, Option[String]) => Node = { (spans, url, title) =>
  ...
  def imageLinkToXHTML: (collection.Seq[Span], String, Option[String]) => Node = { (spans, url, title) =>
    <img src={url} title={title.getOrElse(null)}
